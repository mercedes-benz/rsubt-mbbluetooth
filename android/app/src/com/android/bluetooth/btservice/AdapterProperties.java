...
import static com.android.bluetooth.common.Constants.CAR_LINE_NAME;
...
import com.android.bluetooth.common.CarLineHandler;
import com.android.bluetooth.common.CarLineFactory;
...
class AdapterProperties {
    ...
    // APRICOT-356448 feat: Do not start inquiry after discovery was already started. @{
    private Object mWaitingCallbackLock = new Object();
    private boolean mIsWaitingDiscoveryCallback = false;
    //@}

    // APRICOT-404996: get local manufacturer @{
    // FIXME for QCOM STACK, TEMP SET TO DEFAULT VALUE .
    private volatile int mLocalManufacturer = 29;
    // @}

    // APRICOT-723187: set bluetooth device class @{
    public CarLineHandler mCarLineHandler;
    // @}
    ...
    AdapterProperties(AdapterService service) {
        ...
        mCarLineHandler = CarLineFactory.getCarLineHandler(CAR_LINE_NAME);
    }
    ...
    void onBondStateChanged(BluetoothDevice device, int state) {
        ...
        try {
            ...
            if (state == BluetoothDevice.BOND_BONDED) {
                ...
                // in the Bluetooth adapter where there is no device inquiry
        ...
        for (int i = 0; i < types.length; i++) {
            ...
            synchronized (mObject) {
                switch (type) {
                    ...
                    // APRICOT-404996: get local manufacturer @{
                    case AbstractionLayer.BT_PROPERTY_LOCAL_MANUFACTURER:
                        mLocalManufacturer = Utils.byteArrayToInt(val, 0);
                        intent = new Intent(BluetoothAdapter.ACTION_MANUFACTURER_CHANGED);
                        intent.putExtra(BluetoothAdapter.EXTRA_MANUFACTURER, mLocalManufacturer);
                        intent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
                        mService.sendBroadcast(intent, BLUETOOTH_SCAN,
                                Utils.getTempAllowlistBroadcastOptions());
                        debugLog("mLocalManufacturer: " + mLocalManufacturer);
                        break;
                    // @}
    ...
    void discoveryStateChangeCallback(int state) {
        ...
        synchronized (mObject) {
            // APRICOT-356448 feat: Do not start inquiry after discovery was already started. @{
            setIsWaitingDiscoveryCallback(false);
            // @}
            Intent intent;
    ...
    private void updateRemoteBluetoothClass(BluetoothDevice device) {
        if (!mCarLineHandler.needUpdateRemoteBluetoothClass(device)) {
            infoLog("updateRemoteBluetoothClass: skip update");
            return ;
        }

        DeviceProperties deviceProp = mRemoteDevices.getDeviceProperties(device);
        int cod = deviceProp.getBluetoothClass();
        infoLog("updateRemoteBluetoothClass: device = " + device +  ", cod = " + cod);
        if (AdapterUtil.getDefaultBluetoothClass() == cod ||
                cod == BluetoothClass.Device.Major.MISC) {
            ...
            BluetoothClass bluetoothClass = counterpartDevice.getBluetoothClass();
            if (bluetoothClass == null) {
                errorLog("updateRemoteBluetoothClass: counterpartDevice bluetoothClass is null, return.");
                return;
            }

            int majorDeviceClass = bluetoothClass.getMajorDeviceClass();
            int deviceClass = bluetoothClass.getDeviceClass();
            infoLog("updateRemoteBluetoothClass: counterpartDevice majorClass = "
                    + bluetoothClass.getMajorDeviceClass() + ", deviceClass = "
                    + bluetoothClass.getDeviceClass());

            if (majorDeviceClass == 0 || deviceClass == 0) {
                infoLog("updateRemoteBluetoothClass: counterpartDevice majorDeviceClass is 0, return.");
                return;
            }

            deviceProp.setBluetoothClass(bluetoothClass);
        }
    }
    ...
    protected void dump(FileDescriptor fd, PrintWriter writer, String[] args) {
        ...
        // APRICOT-404996: get local manufacturer @{
        writer.println("  " + "LocalManufacturer: " + mLocalManufacturer);
        // @}
    ...
    // APRICOT-356448 feat: Do not start inquiry after discovery was already started. @{
    public boolean isWaitingDiscoveryCallback() {
        synchronized(mWaitingCallbackLock) {
            debugLog("mIsWaitingDiscoveryCallback="+mIsWaitingDiscoveryCallback);
            return mIsWaitingDiscoveryCallback;
        }

    }
    public void setIsWaitingDiscoveryCallback(boolean isWaitingDiscoveryCallback) {
        synchronized(mWaitingCallbackLock) {
            debugLog("setIsWaitingDiscoveryCallback="+mIsWaitingDiscoveryCallback);
            mIsWaitingDiscoveryCallback = isWaitingDiscoveryCallback;
        }
    ...
    // APRICOT-404996: get local manufacturer @{
    public int getLocalManufacturer() {
        debugLog("getLocalManufacturer: mLocalManufacturer = " + mLocalManufacturer);
        return mLocalManufacturer;
    }
    // @}
}
