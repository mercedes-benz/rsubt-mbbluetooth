...
final class RemoteDevices {
    ...
    // APRICOT-345312: add hci reason for link loss @{
    public static final String KEY_ACL_STATE_HCI_REASON = "hci_reason";
    // @}
    ...
    private static final boolean HFP_BATTERY_CHARGING_SUPPORT =
            SystemProperties.getBoolean("bluetooth.hfp.battery.charging.support", false);
    private static int mChargingState = 0;
    // @}

    private final Handler mHandler;

    // APRICOT-559275 Optimize return values. @{
    private static final int STACK_EVENT_VOLUME_CONTROL_NOT_SUPPORT = 0;
    // @} APRICOT-559275
    ...
    class DeviceProperties {
        ...
        // APRICOT-559275 Optimize return values. @{
        private int mHfpVolumeControlSupportState = BluetoothDevice.VOLUME_CONTROL_SUPPORTED;

        private int mAvrcpVolumeControlSupporteState = BluetoothDevice.VOLUME_CONTROL_SUPPORTED;
        // @} APRICOT-559275
        // @} APRICOT-524456
        ...
        // APRICOT-382703: implement function for AVRCP. @{
        int getUserId() {
            Log.d(TAG, "getUserId return: "+mUserId);
            synchronized (mObject) {
                return mUserId;
            }
        }
        ...
        void setUserId(int userId) {
            Log.d(TAG, "setUserId enter: "+userId);
            synchronized (mObject) {
                mUserId = userId;
            }
        }
        ...
        // APRICOT-356553 feat: Implement function for max connection limit. @{
        int getDisplayId() {
            Log.d(TAG, "getDisplayId return: "+mDisplayId);
            synchronized (mObject) {
                return mDisplayId;
            }
        }
        ...
        void setDisplayId(int displayId) {
            Log.d(TAG, "setDisplayId enter: "+displayId);
            synchronized (mObject) {
        ...
        // APRICOT-559275 Optimize return values. @{
        /**
         * Return whether the remote device support hfp volume control feature.
         *
         * @return
         * {@link BluetoothDevice#VOLUME_CONTROL_UNKNOWN}
         *  if has not been informed by stack;
         * {@link BluetoothDevice#VOLUME_CONTROL_SUPPORTED}
         *  if remote device supports the feature;
         * {@link BluetoothDevice#VOLUME_CONTROL_NOT_SUPPORT}
         *  if remote device does not support the feature;
         */
        int getHfpVolumeControlSupportState() {
            Log.d(TAG, "getHfpVolumeControlSupportState: " + mAvrcpVolumeControlSupporteState);
            return mHfpVolumeControlSupportState;
        }

        /**
         * Return whether the remote device support avrcp absolute volume control feature.
         *
         * @return
         * {@link BluetoothDevice#VOLUME_CONTROL_UNKNOWN}
         *  if has not been informed by stack;
         ...
         *  if remote device does not support the feature;
         */
        int getAvrcpVolumeControlSupportState() {
            Log.d(TAG, "getAvrcpVolumeControlSupportState: " + mAvrcpVolumeControlSupporteState);
            return mAvrcpVolumeControlSupporteState;
        }
        // @} APRICOT-559275
        // @} APRICOT-524456
        ...
        /**
         * @param isCharging the mIsCharging to set
         */
    ...
    private void sendUuidIntent(BluetoothDevice device, DeviceProperties prop) {
        ...
        if (Utils.BLUETOOTH_MULTI_USER_ENABLED) {
            sAdapterService.sendBroadcastAsUser(intent, UserHandle.ALL, BLUETOOTH_CONNECT,
                    Utils.getTempAllowlistBroadcastOptions());
        } else {
            sAdapterService.sendBroadcast(intent, BLUETOOTH_CONNECT,
                    Utils.getTempAllowlistBroadcastOptions());
    ...
    void updateBatteryLevel(BluetoothDevice device, int batteryLevel) {
        ...
        // APRICOT-274970: add charging identifier of bluetooth device @{
        if (HFP_BATTERY_CHARGING_SUPPORT) {
            synchronized (mObject) {
                ...
                boolean currentIsCharging = deviceProperties.isBatteryCharging();
                boolean isCharging = (mChargingState != 0);
                if (isCharging != currentIsCharging) {
                    deviceProperties.setBatteryCharging(isCharging);
                }
            }
        } else {
            synchronized (mObject) {
        ...
        if (HFP_BATTERY_CHARGING_SUPPORT) {
            synchronized (mObject) {
                if (deviceProperties.getBatteryLevel() == BluetoothDevice.BATTERY_LEVEL_UNKNOWN
                        && !deviceProperties.isBatteryCharging()) {
                ...
                deviceProperties.setBatteryCharging(false);
            }
        } else {
            synchronized (mObject) {
                if (deviceProperties.getBatteryLevel() == BluetoothDevice.BATTERY_LEVEL_UNKNOWN) {
    ...
    private void sendBatteryLevelChangedBroadcast(BluetoothDevice device, int batteryLevel) {
        ...
        if (HFP_BATTERY_CHARGING_SUPPORT) {
            DeviceProperties deviceProperties = getDeviceProperties(device);
            boolean isCharging = deviceProperties.isBatteryCharging();
            Log.d(TAG, "sendBatteryLevelChangedBroadcast(), isCharging = " + isCharging);
            intent.putExtra(BluetoothDevice.EXTRA_BATTERY_IS_CHARGING, isCharging);
        }
        // @}
        intent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
            ...
            if (val.length > 0) {
                synchronized (mObject) {
                    ...
                    switch (type) {
                        ...
                        //APRICOT-524456: Support volume control @{
                        // APRICOT-559275 Optimize return values. @{
                        case AbstractionLayer.BT_PROPERTY_REMOTE_HFP_IS_VOLUME_CONTROL_SUPPORTED:
                            if (val[0] == STACK_EVENT_VOLUME_CONTROL_NOT_SUPPORT) {
                                device.mHfpVolumeControlSupportState =
                                        BluetoothDevice.VOLUME_CONTROL_NOT_SUPPORT;
                            } else {
                                device.mHfpVolumeControlSupportState =
                                        BluetoothDevice.VOLUME_CONTROL_SUPPORTED;
                            }

                            debugLog("Remote support hfp volume control ? "
                                    + (boolean) (val[0] != STACK_EVENT_VOLUME_CONTROL_NOT_SUPPORT));
                            // APRICOT-882832 fix: Add volume control support state change callback @{
                            sendVolumeControlStateChangedBroadcast(bdDevice,BluetoothProfile.HEADSET,
                                    device.mHfpVolumeControlSupportState);
                            //}@
                            break;

                        case AbstractionLayer.BT_PROPERTY_REMOTE_AVRCP_IS_VOLUME_CONTROL_SUPPORTED:
                            if (val[0] == STACK_EVENT_VOLUME_CONTROL_NOT_SUPPORT) {
                                device.mAvrcpVolumeControlSupporteState =
                                        BluetoothDevice.VOLUME_CONTROL_NOT_SUPPORT;
                            } else {
                                device.mAvrcpVolumeControlSupporteState =
                                        BluetoothDevice.VOLUME_CONTROL_SUPPORTED;
                            ...
                            debugLog("Remote support avrcp volume control ? "
                                    + (boolean) (val[0] != STACK_EVENT_VOLUME_CONTROL_NOT_SUPPORT));

                            // APRICOT-882832 fix: Add volume control support state change callback @{
                            sendVolumeControlStateChangedBroadcast(bdDevice,BluetoothProfile.A2DP,
                                    device.mAvrcpVolumeControlSupporteState);
                            //}@
                            break;
                            // APRICOT-559275 @}
     ...
     * @param state     :volume control support state
     */
    void sendVolumeControlStateChangedBroadcast(BluetoothDevice device,int profileId,int state) {
        if (device == null) {
            errorLog("device is null");
            return;
        }
        Intent intent = new Intent(BluetoothDevice.ACTION_VOLUME_CONTROL_STATE_CHANGED);
        intent.putExtra(BluetoothDevice.EXTRA_DEVICE, device);
        intent.putExtra(BluetoothDevice.EXTRA_PROFILE_ID, profileId);
        intent.putExtra(BluetoothDevice.EXTRA_VOLUME_CONTROL_STATE, state);
            ...
            for (DiscoveringPackage pkg : packages) {
                ...
                if (Utils.BLUETOOTH_MULTI_USER_ENABLED) {
                    if (pkg.getPermission() != null) {
                        sAdapterService.sendBroadcastAsUserMultiplePermissions(intent,
                                UserHandle.ALL,
    ...
    void aclStateChangeCallback(int status, byte[] address, int newState,
        ...
        if (newState == AbstractionLayer.BT_ACL_STATE_CONNECTED) {
            ...
            if (state == BluetoothAdapter.STATE_ON || state == BluetoothAdapter.STATE_TURNING_OFF) {
                ...
                // APRICOT-345312: add hci reason for link loss @{
                intent.putExtra(KEY_ACL_STATE_HCI_REASON, hciReason);
                // @}
                intent.putExtra(BluetoothDevice.EXTRA_TRANSPORT, transportLinkType);
            } else if (state == BluetoothAdapter.STATE_BLE_ON
                ...
                if (indicatorValueObj instanceof Integer) {
                    if (indicatorType
                        ...
                        indicatorValue = (Integer) indicatorValueObj;
                            ...
                            == BluetoothHeadset.VENDOR_SPECIFIC_HEADSET_EVENT_IPHONEACCEV_BATTERY_CHARGING) {
                        mChargingState = (Integer) indicatorValueObj;
                        Log.d(TAG, "getBatteryLevelFromAppleBatteryVsc(), mChargingState = " + mChargingState);
                    }
                } else {
    ...
    static int getBatteryLevelFromXEventVsc(Object[] args) {
        ...
        if (HFP_BATTERY_CHARGING_SUPPORT) {
            if (args[4] instanceof Integer) {
                mChargingState = (Integer) args[4];
                Log.d(TAG, "getBatteryLevelFromXEventVsc(), mChargingState = " + mChargingState);
            } else {
                Log.w(TAG, "getBatteryLevelFromXEventVsc() error parsing event values of args[4]");
            }
        }
        // @}
